---
layout: default
nav_order: 4
title: Notebooks
nav_exclude: True
---

# Notebooks

[course-files/notebooks/](.)

<table class="tbl-files">
    <tbody>
        <tr>
            <th valign="top"></th>
            <th>Name</th>
            <th>Last modified</th>
            <th>Size</th>
            <th>Download</th>
        </tr>
        <tr>
            <td valign="top"><img src="data:image/png;base64, "
                alt="[PARENTDIR]"></td>
            <td><a href="../">Parent Directory</a></td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
        </tr>

        <tr>
            <td valign="top"><img src="data:image/png;base64, "
                alt="[DIR]"></td>
            <td><a href="scripts">scripts</a></td>
            <td align="right">1/5/2019 3:59 PM</td>
            <td>224.0B</td>
            <td>&nbsp;</td>
        </tr>
        <tr class="click-to-preview">
            <td>
                    <img class="icon-jupyter" src="{{ site.baseurl }}/assets/images/jupyter-icon.png" />
            </td>
            <td nowrap>
                01. Introduction.ipynb<br>
            </td>
            <td align="right">1/3/2019 6:47 PM</td>
            <td>55.9KB</td>
            <td>
                    <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/01. Introduction.ipynb" 
                        target="_blank"><i class="fab fa-github fa-lg"></i></a>
                    <a href="01. Introduction.ipynb"><i class="fas fa-download"></i></a>
            </td>
        </tr>
        <tr class="click-to-preview">
            <td>
                    <img class="icon-jupyter" src="{{ site.baseurl }}/assets/images/jupyter-icon.png" />
            </td>
            <td nowrap>
                02. Primitives Data Types.ipynb<br>
            </td>
            <td align="right">12/30/2018 3:11 AM</td>
            <td>23.5KB</td>
            <td>
                    <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/02. Primitives Data Types.ipynb" 
                        target="_blank"><i class="fab fa-github fa-lg"></i></a>
                    <a href="02. Primitives Data Types.ipynb"><i class="fas fa-download"></i></a>
            </td>
        </tr>
        <tr class="click-to-preview">
            <td>
                    <img class="icon-jupyter" src="{{ site.baseurl }}/assets/images/jupyter-icon.png" />
            </td>
            <td nowrap>
                03. Operators.ipynb<br>
            </td>
            <td align="right">12/29/2018 8:35 PM</td>
            <td>21.7KB</td>
            <td>
                    <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/03. Operators.ipynb" 
                        target="_blank"><i class="fab fa-github fa-lg"></i></a>
                    <a href="03. Operators.ipynb"><i class="fas fa-download"></i></a>
            </td>
        </tr>
        <tr class="click-to-preview">
            <td>
                    <img class="icon-jupyter" src="{{ site.baseurl }}/assets/images/jupyter-icon.png" />
            </td>
            <td nowrap>
                04. Functions.ipynb<br>
            </td>
            <td align="right">12/29/2018 6:48 PM</td>
            <td>15.4KB</td>
            <td>
                    <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/04. Functions.ipynb" 
                        target="_blank"><i class="fab fa-github fa-lg"></i></a>
                    <a href="04. Functions.ipynb"><i class="fas fa-download"></i></a>
            </td>
        </tr>
    </tbody>
</table>

