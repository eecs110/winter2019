---
layout: default
nav_order: 4
title: Notebooks
nav_exclude: True
---

<h1>Notebooks</h1>

<a href=".">course-files/notebooks/</a>

<table class="tbl-files">
    <tbody>
        <tr>
            <th valign="top"></th>
            <th>Name</th>
            <th>Last modified</th>
            <th>Size</th>
            <th>Preview</th>
        </tr>
        <tr>
            <td valign="top"><img src="data:image/png;base64, "
                alt="[PARENTDIR]"></td>
            <td><a href="../">Parent Directory</a></td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
        </tr>

        <tr>
            <td valign="top"><img src="data:image/png;base64, "
                alt="[DIR]"></td>
            <td><a href="scripts">scripts</a></td>
            <td align="right">1/5/2019 3:59 PM</td>
            <td>224.0B</td>
            <td>&nbsp;</td>
        </tr>
        <tr>
            <td>
                <img src="data:image/png;base64, " alt="[DIR]">
            </td>
            <td>
                <i class="fas fa-download"></i> 
                <a href="01. Introduction.ipynb">
                    01. Introduction.ipynb
                </a>
            </td>
            <td align="right">1/3/2019 6:47 PM</td>
            <td>55.9KB</td>
            <td>
                <i class="fab fa-github"></i> 
                <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/01. Introduction.ipynb" target="_blank">
                    preview
                </a>
            </td>
        </tr>
        <tr>
            <td>
                <img src="data:image/png;base64, " alt="[DIR]">
            </td>
            <td>
                <i class="fas fa-download"></i> 
                <a href="02. Primitives Data Types.ipynb">
                    02. Primitives Data Types.ipynb
                </a>
            </td>
            <td align="right">12/30/2018 3:11 AM</td>
            <td>23.5KB</td>
            <td>
                <i class="fab fa-github"></i> 
                <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/02. Primitives Data Types.ipynb" target="_blank">
                    preview
                </a>
            </td>
        </tr>
        <tr>
            <td>
                <img src="data:image/png;base64, " alt="[DIR]">
            </td>
            <td>
                <i class="fas fa-download"></i> 
                <a href="03. Operators.ipynb">
                    03. Operators.ipynb
                </a>
            </td>
            <td align="right">12/29/2018 8:35 PM</td>
            <td>21.7KB</td>
            <td>
                <i class="fab fa-github"></i> 
                <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/03. Operators.ipynb" target="_blank">
                    preview
                </a>
            </td>
        </tr>
        <tr>
            <td>
                <img src="data:image/png;base64, " alt="[DIR]">
            </td>
            <td>
                <i class="fas fa-download"></i> 
                <a href="04. Functions.ipynb">
                    04. Functions.ipynb
                </a>
            </td>
            <td align="right">12/29/2018 6:48 PM</td>
            <td>15.4KB</td>
            <td>
                <i class="fab fa-github"></i> 
                <a href="https://github.com/eecs110/winter2019/blob/master/course-files/notebooks/04. Functions.ipynb" target="_blank">
                    preview
                </a>
            </td>
        </tr>
    </tbody>
</table>

